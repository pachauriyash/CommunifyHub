...
const Card = (props) => {
   ...
   return (
     <div className="card">
       <Link to={linkTo} style={{ color: 'inherit', textDecoration: 'none' , cursor:"pointer"}}>
         <h2 className="card-title">{props.classData.classTitle}</h2>
       </Link>
       <p className="card-description">{props.classData.classDesc}</p>
       <div className="card-footer">
         <span className="card-owner">Admin: {props.classData.classOwner}</span>
         <span className="card-section">Type: {props.classData.classSection}</span>
       </div>
     </div>
   );
 };
